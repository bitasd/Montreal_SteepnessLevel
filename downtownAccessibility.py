...
class NetworkPath:
    ...
    #     """
    #     # dist_dict = self.shortestPath_length(subG, self.source)
    #     dist_dict, path_dict = nx.single_source_dijkstra(G, self.source, target=None, weight="length")
    #     nx.set_node_attributes(self.G, dist_dict, scenario_name)
    #     nx.set_node_attributes(self.G, path_dict, f"{'r_' + scenario_name}")
    #
    # def shortestPath_length(self, G, source, target=None) : # fromOneToMany
    #     try:
    #         # return nx.shortest_path_length(G, source, target, weight='length', method='dijkstra')
    #         return nx.single_source_dijkstra_path_length(G, self.source, target=None, weight="length")  # returns a tuple of dictionaries for paths and length
